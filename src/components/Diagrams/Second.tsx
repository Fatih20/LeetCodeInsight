...
export type SecondDiagram = { solution_type: string; amount: number }[];

ChartJS.register(ArcElement, Tooltip, Legend);

export default function Second() {
  const { data, status } = useQuery<RawDiagramData<SecondDiagram>>({
    queryFn: async () => await diagramDataFetcher<SecondDiagram>(2),
